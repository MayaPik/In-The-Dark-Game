...
public class FlickeringLight : MonoBehaviour {
    ...
    void Start ()
    {
        ...
        intensityOrigin = newLight.intensity;
        rangeOrigin = newLight.range;
        ...
        intensityOffset = newLight.intensity * scale;
        rangeOffset = newLight.range * scale;
    ...
    private void IntensityAndRange()
    {
        ...
        {
            intensityDelta = (intensityOrigin + Random.Range(-intensityOffset, intensityOffset) - newLight.intensity) * speed;


            rangeTarget = rangeOrigin + Random.Range(-rangeOffset, rangeOffset);
            rangeDelta = (rangeTarget - newLight.range) * speed;

            setNewTargets = false;
        }

        newLight.intensity += intensityDelta;
        newLight.range += rangeDelta;


        if (Mathf.Abs(newLight.range - rangeTarget) < 5f * scale) //////// CHECK IF TARGET IS NEAR ENOUGH. /////////
            setNewTargets = true;
